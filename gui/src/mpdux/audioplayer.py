__all__ = ('AudioPlayer', 'AudioPlayerAnnotation')
...
class AudioPlayerPlayPause(Image):
    video = ObjectProperty(None)
...
class AudioPlayerProgressBar(ProgressBar):
    video = ObjectProperty(None)
    seek = NumericProperty(None, allownone=True)
    alpha = NumericProperty(1.)

    def __init__(self, **kwargs):
        super(AudioPlayerProgressBar, self).__init__(**kwargs)
...
class AudioPlayerPreview(FloatLayout):
...
class AudioPlayerAnnotation(Label):
...
class AudioPlayer(GridLayout):
    ...
    '''String, indicates whether to play, pause, or stop the video::
        ...
        video = AudioPlayer(source='movie.mkv', state='play')

        # create the video, and start later
        video = AudioPlayer(source='movie.mkv')
        # and later
        video.state = 'play'
    ...
    the video by setting this property::
        ...
        # start playing the video at creation
        video = AudioPlayer(source='movie.mkv', play=True)
...
#            self._trigger_video_load()
    ...
    def _load_thumbnail(self):
        ...
        #self._image = AudioPlayerPreview(source=thumbnail, video=self)
        ...
        if self._annotations:
            for ann in self._annotations:
                self._annotations_labels.append(
                    AudioPlayerAnnotation(annotation=ann))

    def on_state(self, instance, value):
        Logger.info("AudioPlayer: on_state - instance:{} value:{}".format(instance, value))

        if (value == 'play'):
            Logger.info("AudioPlayer: mpc.play()")
            self.mpc.play()
        if (value == "pause"):
            Logger.info("AudioPlayer: mpc.pause()")
            self.mpc.pause()
        return
        #if self._video is not None:
    ...
    def on_touch_down(self, touch):
        ...
        if touch.is_double_tap and self.allow_fullscreen:
            ...
            return True
        return super(AudioPlayer, self).on_touch_down(touch)

    def on_fullscreen(self, instance, value):
        window = self.get_parent_window()
        if not window:
            Logger.warning('AudioPlayer: Cannot switch to fullscreen, '
                           'window not found.')
            if value:
...
if __name__ == '__main__':
    ...
    player = AudioPlayer(source=sys.argv[1])
    runTouchApp(player)
    if player:
        player.state = 'stop'
