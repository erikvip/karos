...
class WirelessMan(App):

    def get_wireless_interfaces(self):
        Logger.info("WirelessMan: Getting wireless interfaces from /proc/net/wireless")

        try:
            proc = subprocess.Popen("cat /proc/net/wireless  | tail -n +3 | cut -d ':' -f1 | tr -d ' \t'", stdout=subprocess.PIPE, shell=True)
            (output, error) = proc.communicate()
        except:
            Logger.error("WirelessMan: Failed to gather wireless interfaces")
            throw

        inf = [i.strip() for i in output.splitlines()]
        Logger.info("WirelessMan: Found {} wireless interfaces: {}".format(len(inf), inf))
        return inf
    ...
    def build(self):
        ...
        Logger.info("WirelessMan: Using {} for wireless scan".format(inf))
        ...
        Logger.info("WirelessMan: Results from iw scan: {}".format(scan))

        for s in scan:
            Logger.info("WirelessMan: Network info - ssid: {} signal: {} quality: {} bitrates: {} channel: {} encrypted: {}".format(
                    s.ssid, 
                    s.signal, 
                    s.quality, 
                    s.bitrates, 
